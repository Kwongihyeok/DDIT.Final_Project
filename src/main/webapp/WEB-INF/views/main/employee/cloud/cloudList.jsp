...
<!-- 전사 클라우드의 폴더 및 파일 리스트 -->
<c:set var="cloudFolderAndFileList" value="${cloudStrgListMap.cloudFolderAndFileList }"/>

<!-- 개인 클라우드의 커스텀 클라우드함 목록 -->
<c:set var="myCloudStrgCustomList" value="${cloudStrgListMap.myCloudStrgCustomList }"/>	

<!-- 선택한 폴더 또는 클라우드함의 가장 상위 클라우드함 -->
<c:set var="parentCloud" value="${parentCloud }"/>	

<!-- 가져온 폴더 및 파일의 총 카운트 수 -->
<c:set var="selectFldrAndFileCount" value="${selectFldrAndFileCount }"/>	
 
<!-- 선택한 클라우드의 계층형 경로 -->
<c:set var="cloudPathList" value="${cloudStrgListMap.cloudPathList }"/>	
 
<!-- 현재 선택된 클라우드함 또는 폴더의 코드 -->
<c:set var="selectStrgCd" value="${selectStrgCd }"/>	
...
<div id="kt_app_content_container" class="app-container container-fluid" style="height: 100%;">
	<div class="row" style="height: 100%;">
				...
				<div class="modal-dialog modal-dialog-centered mw-650px">
					<div class="modal-content">
						...
						<div class="modal-body pt-5 pb-5 px-lg-17">
						 	<form id="newFldrInsertForm" action="/employee/cloudNewFldrInsert.do" method="post">
						 		<input type="hidden" name="cloudFldrUpCd" value="${selectStrgCd }">
								...
								<div class="card card-flush mb-5 mt-5" style="min-height: 18em; display: flex; justify-content: center; margin-bottom: 1em;">
	                                    ...
	                                    <div class="mt-4 ms-5" style="display: flex; min-height: 6em; padding-top: 1em;">  
	                                       <div class="ms-5" style="width: 20%;">
	                                           <label class="form-label mt-3 fs-3">폴더 이름<font class="ms-2" style="color: red">*</font></label>
	                                       </div>  
	                                       <div style="width: 70%;"> 
	                                        
	                                        <!-- 데이터 세팅하지 않은 기존 코드
											<input type="text" class="form-control" id="newFldrName" name="cloudStrgFldrNm" placeholder="새로운 폴더명을 적어주세요">
	                                        -->
											
											<!-- 발표를 위한 데이터 세팅 -->
											<input type="text" class="form-control fs-3" id="newFldrName" name="cloudStrgFldrNm" value="결재관련폴더">
	                                       
	                                       </div>
	                                    </div>
	                                    <!-- 폴더 이름 끝 -->
	                                    
	                                    <!-- 선택한 클라우드함이 개인 클라우드함이 아닌 경우에만 열람권한 직급 선택 표시하기 -->
	                                    <!-- 직급 출력 시작 -->
	                                    <div id="newFolderCheckParent" class="mt-5 ms-5 mb-3" style="display: flex;">  
	                                       <div class="ms-5" style="width: 20%;">
	                                           <label class="form-label fs-3">공개여부 <font style="color: red">*</font></label>
	                                       </div>  
	                                       <div id="newFolderCheckRow" class="row ms-3" style="width: 70%;"> 
	                                       </div> 
	                                    </div>
	                                   <%--  <c:choose>
	                                    	<c:when test="${parentCloud.cloudKnd ne 'prvt' }">
	                            ...
	                            <div class="mb-3" style="display: flex; justify-content: center;">
	                                <button type="button" class="btn btn-light-primary me-2" onclick="f_newFolderAddBtn()">저장</button>
	                                <button type="button" class="btn btn-light-danger" data-bs-dismiss="modal">취소</button>
	                            </div>
			...
			<div class="modal fade" id="kt_modal_modify_Folder" tabindex="-1" aria-hidden="true">
				<div class="modal-dialog modal-dialog-centered mw-650px">
					<div class="modal-content">
						...
						<!-- 파일 업로드 폼 시작 -->
						<form id="modifyFldrForm" action="/employee/modifyCloudFldr.do" method="post">
			...
			<c:forEach items="${allCloudStrgList }" var="allCloudStrg">
				...
				<c:if test="${allCloudStrg.cloudStrgKnd eq 'dept' }">
					<c:set var="deptCloudStrgVO" value="${allCloudStrg }"/>
				</c:if>
		            ...
		            <div class="card-body" style="padding: 1em;"> 
		                ...
		                <c:if test="${parentCloud.cloudKnd eq 'com' }">
				            <div class="d-flex flex-column">
		                        <div class="d-flex justify-content-between w-100 fs-4 fw-bold mb-5">
 						...
 						<div class="menu-title-gray-700">
							<c:choose>
		                        ...
		                        <div class="menu-link py-3" onclick="f_cloudStrgClick('${deptCloudStrgVO.cloudStrgCd }')">
		                            <span class="menu-icon"> 
		                                <i class="ki-duotone ki-abstract-8 text-success me-3 fs-1"><span class="path1"></span><span class="path2"></span></i>
	                                </span>
	                            	<span class="menu-title fs-4">${deptCloudStrgVO.cloudStrgNm }</span>
		                        </div>
		                    </div>
						</div>
						<!-- 부서 클라우드함 끝 -->
	                ...
	                <div class="card-header mt-3">
	                    ...
	                    <div class="card-title">
	                        <div class="d-flex align-items-center position-relative my-1">
                    			<form action="#" id="searchForm" method="get" style="display: contents;">
	                    ...
	                    <div class="card-toolbar">
	                        <div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
	                            <button type="button" class="btn btn-flex btn-secondary me-1 px-4 callJstree fs-4" data-bs-toggle="modal" data-bs-target="#modalCloudList">
	                                <i class="ki-duotone ki-tablet-text-up fs-2"><span class="path1"></span><span class="path2"></span></i> 
	                                	전체파일목록 
	                            </button>  
	                        </div>  
	                        <div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
	                            <button type="button" class="btn btn-flex btn-light-primary me-1 px-4 fs-4" data-bs-toggle="modal" data-bs-target="#kt_modal_newFolder" onclick="f_cloudNewFlrdJobgradeCall('${parentCloud.cloudKnd}')">
	                                <i class="ki-duotone ki-add-folder fs-2"><span class="path1"></span><span class="path2"></span></i>
	                                	새폴더
	                            </button>
	                        </div> 
	                        <div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
	                            <button type="button" class="btn btn-flex btn-light-success me-1 px-4 fs-4" onclick="f_cloudDownListBtn()">
	                                <i class="ki-duotone ki-folder-down fs-2"><span class="path1"></span><span class="path2"></span></i>
	                                	다운로드
	                            </button>
	                        </div>
	                        <div class="d-flex justify-content-end" data-kt-filemanager-table-toolbar="base">
	                            <button type="button" class="btn btn-flex btn-light-info me-1 px-4 callJstree fs-4" onclick="f_check('move')">
	                        ...
	                        <div class="d-flex justify-content-end align-items-center" data-kt-filemanager-table-toolbar="selected">
	                            <button type="button" class="btn btn-light-danger px-3 fs-4" data-kt-filemanager-table-select="delete_selected" onclick="f_check('del')">
	                                ...
	                                <c:choose>
	                                	<c:when test="${not empty cloudPathList }">
		    	                            <c:forEach items="${cloudPathList }" var="cloudPath" varStatus="i">
				                                <c:if test="${i.index == 0 }">
					                                <a class="fs-5" href="#" onclick="f_cloudPathClick('${cloudPath.cloudCd }')">${cloudPath.cloudNm }</a>
				                                </c:if>
			                                </c:forEach>
		    	                            <c:forEach items="${cloudPathList }" var="cloudPath" varStatus="i">
				                                <c:if test="${i.index != 0 }">
	                                	...
	                                	<c:otherwise>
	                                		<a class="fs-5" href="#" onclick="f_cloudPathClick('${parentCloud.cloudCd }')">${parentCloud.cloudNm }</a>
	                                	</c:otherwise>
	                                </c:choose>
	                            </div>  
	                        </div>
		                            ...
		                            <c:choose>
			                            ...
			                            <c:choose>
				                                 ...
				                                 <tr>
				                                    ...
				                                    <td width="43%" id="cloudNameTd">
				                                        <div class="d-flex align-items-center">
				                                            ...
				                                            <a id="thisALink" href="/employee/cloudList.do?selectStrgCd=${upCloudCode }" class="text-gray-800 text-hover-primary">(상위폴더 이동)</a> 
			                            	...
			                            	<c:if test="${not empty cloudFolderAndFileList }">
			                            		<c:forEach items="${cloudFolderAndFileList }" var="cloudFolderAndFileVO">
				        						   <c:set var="cloudStrgFldrList" value="${cloudFolderAndFileVO.cloudStrgFldrList}" />	
						                                 ...
						                                 <tr>
						                                    ...
						                                    <td width="43%" id="cloudNameTd">
						                                        <div class="d-flex align-items-center">
						                                            ...
						                                            <a id="thisALink" href="/employee/cloudList.do?selectStrgCd=${cloudStrgFldr.cloudStrgFldrCd }" class="text-gray-800 text-hover-primary" onclick="f_fldrLinkBtn('${cloudStrgFldr.cloudStrgFldrCd }', event)">${cloudStrgFldr.cloudStrgFldrNm }</a> 
			                            	...
			                            	<c:if test="${not empty cloudFolderAndFileList }">
			                            		<c:forEach items="${cloudFolderAndFileList }" var="cloudFolderAndFileVO">
				        						   <c:set var="cloudStrgFileList" value="${cloudFolderAndFileVO.cloudFileList}" />	
			                            		
			                            			<c:forEach items="${cloudStrgFileList }" var="cloudStrgFile">
			                            			
				                            		    <tr>
						                                   <td width="7%">
																<div class="form-check form-check-sm form-check-custom form-check-solid" style="display: flex; justify-content: center;">
																	<input id="checkList" class="form-check-input" type="checkbox" value="${cloudStrgFile.cloudFileCd }" />
																</div>
						                                   </td>
						                                   <td width="43%">
						                                       <div class="d-flex align-items-center">
						                                           <a href="/employee/imgPreview.do?cloudFileCd=${cloudStrgFile.cloudFileCd }" target="_blank" class="">
					                                       			...
					                                       			<c:if test="${cloudStrgFile.cloudFileExtnNm eq 'jpg' ||
																				  cloudStrgFile.cloudFileExtnNm eq 'jpeg' ||
																				  cloudStrgFile.cloudFileExtnNm eq 'png' ||
																				  cloudStrgFile.cloudFileExtnNm eq 'gif' }">
																		<a href="/employee/imgPreview.do?cloudFileCd=${cloudStrgFile.cloudFileCd }" target="_blank" class="ki-duotone ki-magnifier ms-2 imgPreview fs-5"> 
																			<span class="path1"></span>
																			<span class="path2"></span>
																		</a>
																	</c:if>
...
$(function() {
	...
	$('#searchBtn').on('click', function() {
		...
		searchForm.attr("action", "/employee/cloudListSearch.do?searchWord=" + keyword);
		searchForm.submit();
	});
	...
	// 엔터키를 눌렀을 때 폴더 생성
	$('#newFldrInsertForm').on('keydown', function(event) {
		if (event.keyCode == 13) {	// Enter 키를 눌렀을 때 (엔터키 == 13번)
		...
		$.ajax({
			...
			success: function(response) {
				...
				if (res.result == "OK") {
					...
					setTimeout(() => {
						location.href = "/employee/cloudList.do?selectStrgCd=" + res.selectStrgCd;
					}, 1000);
					...
					Swal.fire({
						icon: 'warning',
						title: '업로드하려는 폴더에 중복된 이름의 파일이 있습니다!',
						confirmButtonColor: '#4FC9DA',
					});
				}
			}
	...
	$('#customCloudStrg').on('click', "#cloudStrgDeleteBtn", function () {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$.ajax({
					url: "/employee/cloudStrgCustomDelete.do",
					type: "post",
    ...
    $('.callJstree').on('click', function() {
        ...
        $.ajax({
            type: "get",
            url: "/employee/cloudSelectJstree.do",
...
function f_cloudStrgClick(cloudStrgCd) {
	...
	location.href = "/employee/cloudList.do?selectStrgCd=" + selectStrgCd;
}
...
function f_cloudPathClick(strgCd) {
	...
	location.href = "/employee/cloudList.do?selectStrgCd=" + cloudStrgCd;
}
...
function f_cloudStrgAddBtn() {
	...
	$.ajax({
		url: "/employee/cloudStrgCustomInsert.do",
		type: "post",
		data: JSON.stringify(data),
		contentType : "application/json; charset=utf-8",
		success: function(res) {
...
function f_cloudDelListBtn() {
	...
	Swal.fire({
		...
		if (result.isConfirmed) {
			...
			$.ajax({
				url: "/employee/cloudDeleteList.do",
				type: "post",
				data: JSON.stringify(data),
				contentType : "application/json; charset=utf-8",
				success: function(res) {
...
// 새폴더 버튼을 눌렀을 때 (직급 가져오기)
function f_cloudNewFlrdJobgradeCall(cloudKnd) {
	
	let newFolderCheckRow = $('#newFolderCheckRow');
	newFolderCheckRow.html("");

	let html = "";
	
	axios.get("/employee/jobgradeSelectList.do")
	.then(function (res) {
		console.log(res);
		
		if (res.status == 200) {
			let data = res.data;
			data.forEach(jobgradeVO => {
				
				if (cloudKnd != 'prvt') {	// 개인 클라우드함이 아니면 열람권한이 선택 가능하고
					html = `<div class='form-check col-4 mb-5 fs-4'> 
						    	<input class='form-check-input' type='checkbox' value='\${jobgradeVO.jbgdCd}' id='flexCheck' name='cloudFldrAuthrtArray' checked>\${jobgradeVO.jbgdNm}
							</div>`;
				} else {	// 개인 클라우드함이면 열람 권한 선택 없애기
					$('#newFolderCheckParent').css('display', 'none');
					
					html = `<input type='hidden' value='\${jobgradeVO.jbgdCd}' id='flexCheck' name='cloudFldrAuthrtArray' checked>`;
				};
				
				newFolderCheckRow.append(html);
			});
		}
	})
...
// 새폴더 만들기 저장 버튼을 눌렀을 때
function f_newFolderAddBtn() {
	let newFldrInsertForm = $('#newFldrInsertForm');
	let newFldrName = $('#newFldrName').val();
	console.log("새폴더 만들기 저장");
	console.log(newFldrName);
	
	if (newFldrName == null || newFldrName == "") {
		Swal.fire({
			icon: 'warning',
			title: '폴더 이름이 비어있습니다.',
			confirmButtonColor: '#4FC9DA',
	...
	// 폴더명 중복 체크
	axios.get("/employee/newCloudFldrNameCheck.do?cloudFldrName=" + newFldrName)
	.then(function (res) {
		console.log(res);
		...
		if (res.status == 200) {
			...
			data.forEach(CloudFldrAuthrtVO => {
				...
				if (CloudFldrAuthrtVO.cloudFldrAuthrtYn == "Y") {
					...
					console.log("CloudFldrAuthrtVO.jbgdCd : " + CloudFldrAuthrtVO.jbgdCd); 
...
function f_modifyFldr() {
	...
	// 폴더명 중복 체크
	axios.get("/employee/modifyCloudFldrNameCheck.do?cloudFldrName=" + modifyFldrName + "&cloudFldrCd=" + cloudStrgFldrCd)
	.then(function (res) {
		console.log(res);
...
function f_cloudDownListBtn() {
    ...
    let unAuthFldrArr = []; // 권한이 없는 폴더 리스트
    let unAuthFldrArrName = []; // 권한이 없는 폴더 리스트명
    
    // 폴더만 걸러서 배열에 넣기
    let filArr = checkArr.filter(item => item.includes("FDR"));
    console.log("filArr : ");
    console.log(filArr); 
    
    // 모든 axios 요청을 저장할 배열
    let promises = filArr.map(checkFldr => {
        return axios.get("/employee/cloudFldrAuthrtListSelect.do?cloudStrgFldrCd=" + checkFldr)
            .then(function (res) {
                console.log(res);
                ...
                if (res.status == 200) {
                    ...
                    data.forEach(CloudFldrAuthrtVO => {
                        // 열람권한이 있는 직급만 배열에 추가
                        if (CloudFldrAuthrtVO.cloudFldrAuthrtYn == "Y") {
                            jbgdArray.push(CloudFldrAuthrtVO.jbgdCd);
                        }
                    });
                    console.log("jbgdArray : " + jbgdArray);

                    if (!jbgdArray.includes(jbgdCd)) { // 권한이 없는 경우
                        console.log("권한 없음!");
                        unAuthFldrArr.push(checkFldr);
                        console.log("unAuthFldrArr : ", unAuthFldrArr);
                    } else {
                        console.log("권한 있음!");
                    }
                }
    ...
    // 모든 axios 요청이 끝난 후 실행
    Promise.all(promises)
        .then(() => {
            // 권한이 없는 폴더가 있었는지 여부 확인
            if (unAuthFldrArr.length == 0) {
                console.log("다운 가능함!");
                // 다운로드 요청, map 형식으로 데이터 전송
                axios.post("/employee/cloudDownList.do", {
                    downList: checkArr
                })
                .then(function (res) {
            ...
            } else { // 권한이 없는 폴더가 하나라도 있었다면
                // 폴더만 걸러서 배열에 넣기
                let filterArray = unAuthFldrArr.filter(item => item.includes("FDR"));
                console.log("filterArray : "); 
                console.log(filterArray); 
                
                let data = {
                    checkArr: filterArray
                };
                
                $.ajax({
                    url: "/employee/unAuthFldrListSelect.do",
                    type: "post",
                    data: JSON.stringify(data),
                    contentType: "application/json; charset=utf-8",
                    success: function(res) {
                        res.forEach(flrdVO => {
                            // 중복 체크 후 추가
                            if (!unAuthFldrArrName.includes(flrdVO.cloudStrgFldrNm)) {
                                unAuthFldrArrName.push(flrdVO.cloudStrgFldrNm);
                            }
                        });
                        
                     	// 배열을 이름순으로 정렬
                        unAuthFldrArrName.sort();
                        ...
                        Swal.fire({
                            icon: 'warning',
                            title: '열람권한이 없는 폴더가 있습니다!',
                            text: unAuthFldrArrName.join(', '),
                            confirmButtonColor: '#4FC9DA',
                        });
                    }
                });
...
// 폴더를 클릭했을 때 열람 권한이 있는지 확인
function f_fldrLinkBtn(fldrCd, e) {
	e.preventDefault();
	// 로그인한 사원의 직급코드
	console.log("jbgdCd : " + jbgdCd);
...
function f_fldrLocateSelectBtn() {
	...
	axios.get("/employee/cloudFldrAuthrtListSelect.do?cloudStrgFldrCd=" + strgSelectCd)
	.then(function (res) {
		console.log(res);
		...
		if (jbgdArray.includes(jbgdCd)) {
			console.log("권한 있음!");
			// 권한이 있으면 폴더 접속   
			location.href="/employee/cloudList.do?selectStrgCd=" + strgSelectCd;
		} else {
			Swal.fire({
				icon: 'warning',
				title: '폴더 열람 권한이 없습니다!',
