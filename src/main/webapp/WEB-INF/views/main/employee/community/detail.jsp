...
<div class="mb-10" id="kt_social_feeds_posts">
			...
			<div class="d-flex ">
				<div
					...
					<p class="text-gray-500 fw-semibold d-block"
						style="margin-top: auto;">${communityVO.empNm }</p>
					<!--end::Date-->
	...
	<div>
		<input type="hidden" id="listType" value="${communityVO.cmntyType }" />
...
$(function() {
	...
	downloadBtn.on("click", function(){
	    ...
	    if (selectedFiles.length === 0) {
		  ...
		  $.ajax({
		        url: "/employee/downloadAtchFile?path=" + atchFileDetailStrgNm,
		        type: "post",
		        processData: false,
		        contentType: false,
		        data: formData,
	...
	modifyAdd.on("click", function(){
	    ...
	    $.ajax({
	        url: "/employee/communityModify/1",
	        type: "post",
	        processData: false,
	        contentType: false,
	        data: formData,
       	...
       	Swal.fire({
			...
			if (result.isConfirmed) {
				...
				let data = {
				   ...
				   $.ajax({
				        url: "/employee/communityRemove.do", // 올바른 cmntyNo 사용
				        type: "post",
				        data: JSON.stringify(data),
						contentType : "application/json; charset=utf-8",
				        success: function(res) {
