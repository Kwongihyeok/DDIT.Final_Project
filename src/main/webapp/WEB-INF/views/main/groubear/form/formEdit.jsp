...
<form action="/groubear/formInsert.do" id="formInsert" method="post">
   <c:if test="${status eq 'u' }">
                                         ...
                                         <div class="card-title py-5" style="width: 100%;">
                                            <div style="width: 100%; display: flex; justify-content: space-between;">
                                               <div style="display: flex; align-items: center;">
                                                   <h2><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">양식 ${insertAndUpdate }</font></font></h2>
                                               </div>
                                         ...
                                         <div class="mb-10 fv-row fv-plugins-icon-container">
                                               ...
                                               <div style="width: 20%;"> 
	                                                  <select id="docFormKndNm" name="docFormKndUp" class="form-select fs-3" data-control="select2" data-hide-search="true">
	                                                     ...
	                                                     <c:forEach items="${docFormKndNmList }" var="docFormNm"> 
	                                               		 <c:set var="docFormDetailNm" value="ㄴ${formVO.docFormKndNm }"/>
	                                                     	<c:choose>
	                                                     		<c:when test="${formVO.docFormKndNm == docFormNm.docFormNm }">
		                                                        	<option value="${docFormNm.docFormNo }" selected="selected">${docFormNm.docFormNm }</option>
	                                                     		</c:when> 
		                                                     	<c:when test="${docFormDetailNm == docFormNm.docFormNm}">
			                                                        <option value="${docFormNm.docFormNo }" selected="selected">${docFormNm.docFormNm }</option>
	                                                     		</c:when> 
	                                                     		<c:otherwise>
                                            ...
                                            <div>
                                             ...
                                             <div class="row mt-5">
	                                            ...
	                                            <!--CKEditor 시작-->
	                                            <div class="col-7" style="width: 70%; min-width: 720px; display: flex; justify-content: center; background-color: #dbdbdb; border: 1px solid #f8f8f8;">
...
$(function() {
   ...
   $("#myInput").on("keyup", function() {
      ...
      // tbody의 tr 요소만 필터링
      $("#kt_ecommerce_products_table tbody tr").filter(function() {
...
// ***** form Process *****
   ...
   addBtn.on('click', function() {
      ...
      // 수정일 때
      if ($(this).val() == '수정') {
         formInsert.attr("action", "/groubear/formUpdate.do");
      }
   ...
   deleteBtn.on('click', function() {
      Swal.fire({
         ...
         if (result.isConfirmed) {
            formInsert.attr("action", "/groubear/formDelete.do");
            formInsert.submit();
         }
      })
   });
...
function f_createJstree(treeData) {
    ...
    $('#formJstree').bind('select_node.jstree', function(event, data){
       ...
       $.ajax({
          url: "/groubear/selectForm.do?docFormNo=" + docFormNo,
          type: "get",
