...
<div id="kt_app_content_container" class="app-container container-fluid mb-15" style="height: 100%;"> 
	<div style="height: 100%;">
        ...
        <div class="p-0 mb-2" style="height: 85%">  
	        <div class="card card-flush" style="height: 103%">
					...
					<div class="card-title" style="width: 100%; justify-content: space-between"> 
						<div class="d-flex">
							<div class="d-flex align-items-center position-relative my-1">
			                 	...
			                 	<form id="searchForm" style="display: contents;">
			                        ...
			                        <input type="hidden" name="cmntyQstnSttsYn" id="cmntyQstnSttsYn">
								...
								<c:when test="${empty cmntyType }">
									<div>
										<div class="form-check form-check-sm form-check-custom form-check-solid ms-10 mt-7" style="display: flex; justify-content: center;">
											<c:choose>
												<c:when test="${empty cmntyQstnSttsYn }">
													<input class="form-check-input me-3" type="checkbox" onclick="f_cmntyMyQstnBtn(this)" />
												</c:when>
												<c:otherwise>
													<input class="form-check-input me-3" type="checkbox" checked="checked" onclick="f_cmntyMyQstnBtn(this)" />
												</c:otherwise>
											</c:choose>
											<div class="text-start text-gray-500 fw-bold fs-5 text-uppercase gs-0 text-center">대기중만 보기</div>  
										</div>
									</div> 
								</c:when>
								<c:otherwise>
						...
						<c:choose>
							<c:when test="${cmntyType eq 'QSTNKND001' }">
								<span class="badge badge-light-primary float-left badge-lg fs-3">${qstnCountService.cmntyQstnSttsNCount } / ${qstnCountService.cmntyQstnSttsNCount + qstnCountService.cmntyQstnSttsYCount}</span>
							</c:when>
							<c:when test="${cmntyType eq 'QSTNKND002' }">
								<span class="badge badge-light-primary float-left badge-lg fs-3">${qstnCountPrice.cmntyQstnSttsNCount } / ${qstnCountPrice.cmntyQstnSttsNCount + qstnCountPrice.cmntyQstnSttsYCount}</span>
							</c:when>
							<c:when test="${cmntyType eq 'QSTNKND003' }">
								<span class="badge badge-light-primary float-left badge-lg fs-3">${qstnCountOther.cmntyQstnSttsNCount } / ${qstnCountOther.cmntyQstnSttsNCount + qstnCountOther.cmntyQstnSttsYCount}</span>
							</c:when>
							<c:otherwise>
								<span class="badge badge-light-primary float-left badge-lg fs-3">${qstnCountAll.cmntyQstnSttsAllN } / ${qstnCountAll.cmntyQstnAllcount}</span>
							</c:otherwise>
						</c:choose>
										...
										<tr>
											...
											<td class="py-3">
												<div class="text-gray-800 px-5" style="display: flex; justify-content: left;">
													<div class="text-center">
		 												<a href="/groubear/cmntyQstnDetail.do?cmntyNo=${cmntyQstnVO.cmntyNo }">${cmntyQstnVO.cmntyTtl }</a>
...
// 대기 중만 보기 버튼 눌렀을 때
function f_cmntyMyQstnBtn(checkbox) {
    if (checkbox.checked) { // 체크가 되어 있다면
        // 대기 중만 보기 상태로 설정
        searchForm.find("#cmntyQstnSttsYn").val('N');
    } else { // 체크가 해제되었다면
        // 값을 제거하고 전체 보기 상태로 설정
        searchForm.find("#cmntyQstnSttsYn").remove();
    }
    searchForm.submit();
}	
